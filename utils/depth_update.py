...
def sliding_error_tolerant_fragment(curr_tree_depth, gt_depth_img, b_tree, depth_start, depth_end, is_first):
    with torch.no_grad():
        ...
        if curr_tree_depth == 2:
            ...
            modified_label = gt_label.clone()  # 确保这行在所有分支中都有执行
            # 更新 gt_label 的值
            ...
            # 只保留gt_label为 0,1,2,3 的部分为True的 bin_mask
        ...
        with torch.no_grad():
            ...
            if curr_tree_depth  == 1:
                ...
                for i in range(4):
                    ...
                    tmp_key1 = tmp_key1.long()  # 如果 tmp_key1 是浮点型且需要 Long 类型
...
def depthmap2tree(depth_img, tree_depth, depth_start, depth_end, scale_factor=1.0, mode='bilinear', with_grad=False, no_detach=False):
    if not with_grad:
        with torch.no_grad():
            ...
            b_tree[:, 1, :, :] = (torch.floor((depth_img - depth_start) / d_interval)).type(torch.int64) # 当前阶段深度值所在的片段位置（位于第几个片段）
