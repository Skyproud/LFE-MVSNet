...
class WTD_Conv2d_WOdown(nn.Module):
    def __init__(self, in_channels, out_channels, bn_momentum=0.1):
        ...
        self.depthwise = WTConv2d(in_channels, in_channels, kernel_size=5, wt_levels=2)
        self.pointwise = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1,
                              bias=False)
        self.norm_layer = nn.BatchNorm2d(out_channels, momentum=bn_momentum)
...
class DeformStageNet(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        self.conv0 = nn.Sequential(
            # Conv2dUnit(3, base_channels, 3, 1, padding=1),
            Down_wt(3, base_channels),
            Conv2dUnit(base_channels, base_channels, 3, 1, padding=1),
            # WTD_Conv2d_WOdown(3, base_channels)
        )
        # [B,16,H/2,W/2]
        self.conv1 = nn.Sequential(
            # WTD_Conv2d(base_channels, base_channels * 2),
            # Conv2dUnit(base_channels, base_channels * 2, 5, stride=2, padding=2),
            Down_wt(base_channels, base_channels * 2),
            Conv2dUnit(base_channels * 2, base_channels * 2, 3, 1, padding=1),
            Conv2dUnit(base_channels * 2, base_channels * 2, 3, 1, padding=1),
        )
        # [B,32,H/4,W/4]
        self.conv2 = nn.Sequential(
                # WTD_Conv2d(base_channels * 2, base_channels * 4),
                # Conv2dUnit(base_channels * 2, base_channels * 4, 5, stride=2, padding=2),
                Down_wt(base_channels * 2, base_channels * 4),
                Conv2dUnit(base_channels * 4, base_channels * 4, 3, 1, padding=1),
                Conv2dUnit(base_channels * 4, base_channels * 4, 3, 1, padding=1),
        )
        ...
        if stage_num == 4:
            # [B,64,H/8,W/8]
            self.conv3 = nn.Sequential(
            # WTD_Conv2d(base_channels * 4, base_channels * 8),
            # Conv2dUnit(base_channels * 4, base_channels * 8, 5, stride=2, padding=2),
            Down_wt(base_channels * 4, base_channels * 8),
            Conv2dUnit(base_channels * 8, base_channels * 8, 3, 1, padding=1),
            Conv2dUnit(base_channels * 8, base_channels * 8, 3, 1, padding=1),
            )
...
class Reg2d(nn.Module):
    def __init__(self, input_channel=128, base_channel=32):
        ...
        self.conv1 = ConvBnReLU3D(base_channel, base_channel * 2, kernel_size=(1, 3, 3), stride=(1, 2, 2),
                                  pad=(0, 1, 1))
        self.conv2 = ConvBnReLU3D(base_channel * 2, base_channel * 2)

        self.conv3 = ConvBnReLU3D(base_channel * 2, base_channel * 4, kernel_size=(1, 3, 3), stride=(1, 2, 2),
                                  pad=(0, 1, 1))
        self.conv4 = ConvBnReLU3D(base_channel * 4, base_channel * 4)

        self.conv5 = ConvBnReLU3D(base_channel * 4, base_channel * 8, kernel_size=(1, 3, 3), stride=(1, 2, 2),
                                  pad=(0, 1, 1))
...
if __name__ == '__main__':
    ...
    x2 = torch.randn(1, 16, 64, 80) # input B C H W
