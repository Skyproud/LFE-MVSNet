...
class WT_Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1):
        super(WT_Conv2d, self).__init__()
        self.padding = padding
        self.kernel_size = kernel_size
        self.stride = stride
        self.depthwise = WTConv2d(in_channels, in_channels, kernel_size=5)
        self.pointwise = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride,
...
class Res_Unit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding=1):
        ...
        self.conv = WT_Conv2d(in_channels, out_channels, kernel_size, stride, padding)
        self.kernel_size = kernel_size
        self.stride = stride
...
class DeformStageNet(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        self.base_channels = base_channels
        self.stage_num = stage_num
        self.esam0 = ESAM(in_channels=base_channels * 2)
        self.esam1 = ESAM(in_channels=base_channels * 4)
    ...
    def forward(self, x):
        ...
        # esam0 = self.esam0(conv0)
        conv1 = self.conv1(conv0) # [B,16,H/2,W/2]
        # esam1 = self.esam1(conv1)
        conv2 = self.conv2(conv1) # [B,32,H/4,W/4]

        if self.stage_num == 4:
            conv3 = self.conv3(conv2)  # 64,H/8,W/8
            output_feature["0"] = self.conv_out["0"](conv3)
            intra_feat1 = F.interpolate(conv3, scale_factor=2, mode="bilinear") + self.conv_inner["1"](conv2) # 64,H/4,W/4
            output_feature["1"] = self.conv_out["1"](intra_feat1) # 32,H/4,W/4
            intra_feat2 = F.interpolate(conv2, scale_factor=2, mode="bilinear") + self.conv_inner["2"](conv1) # 32,H/2,W/2
            esam1 = self.esam1(intra_feat2)
            fusion_feature = esam1 + intra_feat2
            output_feature["2"] = self.conv_out["2"](fusion_feature)
            intra_feat3 = F.interpolate(conv1, scale_factor=2, mode="bilinear") + self.conv_inner["3"](conv0) # 16,H,W
            esam0 = self.esam0(intra_feat3)
            fusion_feature = esam0 + intra_feat3
            output_feature["3"] = self.conv_out["3"](fusion_feature)
...
class SC_StageNet1(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        # [B,8,H,W]
        self.conv0 = Res_Unit(3, base_channels, 3, 1, padding=1)
        # [B,16,H/2,W/2]
        self.conv1 = Res_Unit(base_channels, base_channels * 2, 3, stride=2, padding=1)
        # [B,32,H/4,W/4]
        self.conv2 = Res_Unit(base_channels * 2, base_channels * 4, 3, stride=2, padding=1)

        if stage_num == 4:
            # [B,64,H/8,W/8]
            # self.conv3 = Res_Unit(base_channels * 4, base_channels * 8, 3, stride=2, padding=1)
            self.conv3 = nn.Sequential(
                Conv2dUnit(base_channels * 4, base_channels * 8, 5, stride=2, padding=2),
                Conv2dUnit(base_channels * 8, base_channels * 8, 3, 1, padding=1),
                Conv2dUnit(base_channels * 8, base_channels * 8, 3, 1, padding=1),
...
class SC_StageNet(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        self.conv0 = Res_Unit(3, base_channels, 3, 1, padding=1)
        # [B,16,H/2,W/2]
        self.conv1 = Res_Unit(base_channels, base_channels * 2, 5, stride=2, padding=2)
        # [B,32,H/4,W/4]
        self.conv2 = Res_Unit(base_channels * 2, base_channels * 4, 5, stride=2, padding=2)

        if stage_num == 4:
            # [B,64,H/8,W/8]
            self.conv3 = Res_Unit(base_channels * 4, base_channels * 8, 5, stride=2, padding=2)
...
if __name__ == '__main__':
    x1 = torch.randn(1, 64, 128, 160)
    x2 = torch.randn(1, 64, 64, 80) # 输入 B C H W
    model = WT_Conv2d(8, 16, 3, stride=2,padding=1)
    model1 = Res_Unit(8, 16, 3, 2, padding=1)
