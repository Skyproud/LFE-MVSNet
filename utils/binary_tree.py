...
def update_tree1(binary_tree, indicator, direction):
    ...
    """
    tree_list = []
    tree_list.append(torch.clamp_min(binary_tree[:, 0, :, :] + torch.squeeze(indicator, 1), 0)) # 每次加1，值为1,2,3...,8
    tree_key = torch.clamp_min((binary_tree[:, 1, :, :] * \
        (2.0 ** torch.squeeze(indicator, 1))).type(torch.int64) + \
        torch.squeeze(direction, 1), 0)
    # print(tree_list[0].shape)
    # print(tree_key)
    tree_key = torch.minimum(tree_key, 2 ** (tree_list[0] + 2) - 1) # clamp max, should be 2 ** tree_list[0] - 1
    # print(tree_key)
