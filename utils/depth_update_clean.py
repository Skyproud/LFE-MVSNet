        ...
        with torch.no_grad():
            ...
            if depth_start.dim() == 3:
                if depth_start.shape[1] != pred_label.shape[1] or pred_label.shape[2] != pred_label.shape[2]:
                    ...
                    depth_start = F.interpolate(depth_start, size=[pred_label.shape[1], pred_label.shape[2]],
                                                mode="nearest")
                    depth_start = torch.squeeze(depth_start, 1)

                    depth_end = torch.unsqueeze(depth_end, 1)
                    depth_end = F.interpolate(depth_end, size=[pred_label.shape[1], pred_label.shape[2]],
                                              mode="nearest")
