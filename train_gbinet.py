...
def train_model_stage(model,
    ...
    for iteration, sample in enumerate(data_loader):
        ...
        for curr_tree_depth in range(1, max_tree_depth + 1):
            ...
            pred_label = outputs["pred_label"]  # (1,64,80)
            # print(pred_label)
            ...
            sample_cuda["binary_tree"]["depth"], sample_cuda["binary_tree"]["tree"] = \
                depth_update.update_4pred_4sample(gt_label, curr_tree_depth, sample_cuda["binary_tree"]["tree"],
                                           torch.unsqueeze(pred_label, 1), depth_start=depth_min_max[:, 0],
                                           depth_end=depth_min_max[:, 1], is_first=is_first)

            # print(sample_cuda["binary_tree"]["depth"])
            ...
            scalar_outputs["accu0"] = GBiNet_accu(pred_label, gt_label,
                                                  torch.logical_and(torch.eq(gt_label, 0), mask > 0.0))
            scalar_outputs["accu1"] = GBiNet_accu(pred_label, gt_label,
                                                  torch.logical_and(torch.eq(gt_label, 1), mask > 0.0))
            scalar_outputs["accu2"] = GBiNet_accu(pred_label, gt_label,
                                                  torch.logical_and(torch.eq(gt_label, 2), mask > 0.0))
            scalar_outputs["accu3"] = GBiNet_accu(pred_label, gt_label,
                                                  torch.logical_and(torch.eq(gt_label, 3), mask > 0.0))
    ...
    if tensorboard_logger is not None and my_rank == 0:
        for curr_tree_depth in range(1, max_tree_depth + 1):
            for key, value in avg_train_scalars["depth{}".format(curr_tree_depth)].mean().items():
                tensorboard_logger.add_scalars("Avg_" + key + "_depth{}".format(curr_tree_depth), {"train": value},
                                               curr_epoch)
            logger.info("Epoch {} depth {} average train scalars {}".format(curr_epoch, curr_tree_depth,
                                                                            avg_train_scalars["depth{}".format(
                                                                                curr_tree_depth)].mean()))
...
def train(rank, cfg):
    ...
    train_dataset = MVSDataset(cfg["data"]["train"]["root_dir"], 
        ...
        depth_num=cfg["data"]["train"].get("depth_num", 16),
    ...
    val_dataset = ValMVSDataset(cfg["data"]["val"]["root_dir"], 
        ...
        depth_num=cfg["data"]["val"].get("depth_num", 16))
...
def main():
    ...
    parser.add_argument("--cfg", dest="config_file", default="/home/d6813/LZHL/AEFT-GBI/configs/train_dtu_crop.yaml", metavar="FILE", help="path to config file", type=str)
