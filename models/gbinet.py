...
from modules.gbinet_submodules import StageFeatExtNet, CostRegNet, CostRegNetBN, PixelwiseNet,FeatureFetcher, get_pixel_grids, Reg2d
...
class GBiNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.feat_name = cfg["model"].get("feat_name")
    ...
    def forward_all_depth(self, data_batch, max_tree_depth, is_clean=True, out_depths=None, prob_out_depths=None):
        ...
        for curr_tree_depth in range(1, max_tree_depth + 1):
            ...
            pred_feature = self.cost_network[str(stage_id)](cost_img, stage_id)
            pred_feature = torch.squeeze(pred_feature, 1)
...
def GBiNet_loss(preds, modified_label, mask):
    # preds: B, C, H, W
    # gt_label: B, H, W
    # mask: B, H, W
    mask = mask > 0.0 # B, H, W
    ...
    gt_label_mask = modified_label[mask] # N
